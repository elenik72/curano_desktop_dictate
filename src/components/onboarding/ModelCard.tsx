...
const ModelCard: React.FC<ModelCardProps> = ({
  ...
  const baseClasses =
    "flex flex-col gap-2 rounded-[22px] bg-white px-5 py-4 text-left transition-all duration-200 shadow-[0_10px_30px_rgba(15,23,42,0.04)]";

  const getVariantClasses = () => {
    if (status === "active") {
      return "border-2 border-red-200 bg-red-50/60";
    }
    if (isFeatured) {
      return "border-2 border-red-100 bg-red-50/30";
    }
    return "border-2 border-slate-200";
  };

  const getInteractiveClasses = () => {
    if (!isClickable) return "";
    if (disabled) return "opacity-50 cursor-not-allowed";
    return "group cursor-pointer hover:border-red-200 hover:bg-red-50/30 hover:shadow-[0_18px_40px_rgba(15,23,42,0.08)] hover:scale-[1.01] active:scale-[0.99]";
  };
      ...
      <div className="flex justify-between items-center w-full">
        <div className="flex flex-col items-start flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            <h3
              className={`text-base font-semibold text-slate-900 ${isClickable ? "group-hover:text-red-600" : ""} transition-colors`}
            >
              {displayName}
            </h3>
          ...
          <div className="hidden sm:flex items-center ms-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <p className="w-24 text-end text-xs text-slate-500">
      ...
      <div className="flex items-center gap-3 w-full -mb-0.5 mt-0.5 h-5">
        ...
        {onDelete && (status === "available" || status === "active") && (
          <Button
            ...
            className="ms-auto flex items-center gap-1.5 text-red-600 hover:bg-red-50 hover:text-red-700"
          >
